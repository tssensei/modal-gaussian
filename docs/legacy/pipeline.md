# Archived pipeline documentation

This is a historical snapshot, not the current execution guide. Use the repository [README](../../README.md) and [BASELINE](../../BASELINE.md) for new work. Farneback, greedy frequency selection and selected-frequency DFT execution now require the explicit `legacy` CLI prefix. Other recipes below may describe superseded research variants. Commands and example paths are relative to the repository root. Do not rerun validation or modify saved artifacts because an archived example says to do so.
...
## Shared-grid spectrum browser

The standalone browser reads cached complex U/V images without loading a 3D model.
It supports manual sample selection, frequency zoom, U/V previews, pixel readouts,
and exporting selected frequencies. There is no automatic peak search or snapping:
an entered frequency must already belong to the cache grid. DC is display-only.

Corn's existing SEA-RAFT sequences are sampled at 20 fps. Both views use a
1600-point FFT (0–10 Hz, 0.0125 Hz spacing, 0.225 Hz at bin 18). Temporal mean
subtraction and the symmetric Hann window use each original sequence length,
before zero padding. All image pixels are cached; the projected manual subject
box only selects the default spectrum/display region and includes background.
...
Open `http://127.0.0.1:8110/` after starting the viewer. The build command reuses
a complete matching cache; it never recomputes optical flow. The two full-frame
complex spectra total approximately 22 GiB before compression, processed in tiles.
Build does not start the browser, train modes, or run experiment validation.

Save writes a new selection JSON under the viewer work directory. Export writes
that selection and `modal_images/bin_XXXX/view1` (and `view2`) directories, each
containing `modal_image.npy` plus provenance. These are direct copies of cached
slices, not another DFT. The equivalent CLI export is:

```bat
modal-gaussians spectrum export --input outputs\corn_spectrum_001 --selection <saved-selection.json> --output <new-export-directory>
```

Pass the per-view exported directories to the existing `motion prepare-selected-modal`
and `graph build-modal-similarity` `--view` arguments, with the exported exact
frequency. Preparation can introduce frequencies absent from its parent snapshot;
it reuses geometry and recomputes complex view alignment. A graph from another
frequency must not be reused as that frequency's soft-weight graph. The old greedy
selection command is unchanged and does not automatically use this new cache.
