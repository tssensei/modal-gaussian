...
First record interpreter, package import path, Python/PyTorch/CUDA/gsplat/Viser versions, GPU name, `nvcc --version`, `pip check`, and resolved COLMAP binary. The target is Python 3.11, PyTorch 2.7.1+cu128, toolkit/runtime 12.8, gsplat 1.5.3, Viser 1.0.30 from this repository's environment definition. CUDA runtime and toolkit must both match 12.8; the GPU driver's advertised maximum CUDA version may be newer.
...
assert importlib.metadata.version("viser") == "1.0.30"
...
Importing gsplat alone is not this probe. If it fails, diagnose interpreter/PATH, toolkit, compiler, extension build and GPU compatibility before spending time on the real pipeline. Never relax the CUDA 12.8 constraint to obtain a passing import.
...
The two Python blocks are probes/templates, not persistent test-suite additions. Use project source if these internal APIs change; do not make the implementation conform to stale probe assumptions.
...
| COLMAP fails | Capture the command output immediately: its temporary workspace/log is removed in `finally`, even on failure. The exception includes only a log tail. If more evidence is needed, make a narrowly scoped logging fix before retrying; do not assume its failed workspace survives. |
...
| Out of memory / disk / walltime | Measure the actual bottleneck. Use authorized additional resources or a behavior-preserving implementation fix where justified. Changes to K, images, resolution, epochs, graph thresholds or scientific settings require direction. Even performance-only CLI changes must be checked for identity/resume effects. |
