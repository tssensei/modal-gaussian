...
Historical snapshot only. See [current commands](../commands.md). Old Farneback and frequency-compute commands need the explicit `legacy` prefix. Old validation checklists are overridden by the current no-validation policy. Example paths remain relative to the repository root.
