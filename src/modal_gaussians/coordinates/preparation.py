...
from modal_gaussians.coordinates.design import RenderedDesignConfig, RenderedDesignViewInput, build_rendered_modal_design_artifact, load_rendered_modal_design
...
def _mode_sources(index_path, status, expected_modes):
    ...
    for record in selected:
        ...
        if (header.get("version") != 18 or header.get("format") != COMPLETED_MODES_FORMAT
                or header.get("completed_modes_identity") != record["completed_modes_identity"]):
            raise ValueError("Index must bind an unchanged completed v18 model")
...
def _bank_contract(sources, scene_manifest, flows):
    from modal_gaussians.motion import artifacts, network as neural_field, component_field
    from modal_gaussians.coordinates import sources as coefficient_sources
...
def build_mode_bank(*, sources, scene_dir, flows, output_dir):
    """Copy saved Phi unchanged; copy saved angular fields."""
        ...
        try:
            ...
            for i, source in enumerate(sources):
                ...
                if artifact.rotation is None:
                    raise ValueError("v18 spatial modes must provide their saved-model rotation field")
...
def build_initial_coordinates(*, design, flows, output_dir, config):
    ...
    for view, flow in zip(design.manifest["views"], flows):
        ...
        q, diagnostics = solve_direct_coordinates_view(
            ...
            fps_hz=flow.manifest["fps_hz"], frequencies_hz=frequencies, config=config)
...
def prepare_coefficient_inputs(*, scene, output_dir, expected_modes, status,
    ...
    from modal_gaussians.coordinates import direct as direct_coordinates, design as rendered_design
