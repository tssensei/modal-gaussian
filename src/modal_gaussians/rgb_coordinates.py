...
def _image_directory(flow_dir: str | Path, view: Mapping[str, Any]) -> Path:
    """Read source metadata only; never open old flow or spectrum arrays."""
    root = resolve_path(flow_dir, strict=True)
    manifest = json.loads((root / "manifest.json").read_text(encoding="utf-8"))
    if manifest.get("format") in ("modal_gaussians.sea_raft_flow",
        ...
        flow = load_coordinate_flow(root)
    ...
    if manifest.get("format") != "modal_gaussians.flow_analysis":
        raise ValueError("RGB auto-discovery requires the direct artifact's original flow metadata")
    for name in ("frame_names", "fps_hz", "reference_frame_name", "reference_frame_index"):
        if manifest.get(name) != view[name]:
            raise ValueError(f"RGB source {name} differs for view {view['label']!r}")
    stable = manifest.get("stabilized_sequence")
    if stable is not None:
        stable_root = (root / stable["path"]).resolve(strict=True)
        if not stable_root.is_relative_to(root):
            raise ValueError("Stabilized images must stay within their source artifact")
        metadata = json.loads((stable_root / "manifest.json").read_text(encoding="utf-8"))
        if (metadata["frames"] != view["frame_names"] or metadata["fps_hz"] != view["fps_hz"]
                or metadata["reference_frame"] != view["reference_frame_name"]):
            raise ValueError("Stabilized image timing differs from the direct coordinates")
        return stable_root / "images"
    return resolve_path(manifest["inputs"]["sequence"]["image_directory"], strict=True)
...
def build_rgb_modal_coordinates_artifact(
    *,
    ...
    from modal_gaussians.result import _load_sources, _select_views
