...
from modal_gaussians.coordinates.rgb import load_rgb_frame
...
def frame_metrics(prediction, target, perceptual=None):
    ...
    if perceptual is not None:
        values['lpips'] = float(perceptual(prediction.permute(2, 0, 1)[None],
                                          target.permute(2, 0, 1)[None], normalize=True).item())
...
def evaluate_result(*, result_dir, output_dir, view_labels=None, with_lpips=False, device='cuda', baseline_dir=None):
    ...
    protocol = dict(task='fitted_recording_reconstruction', region='full_frame',
                    ...
                    ssim='pytorch-msssim; window 11, sigma 1.5, data_range 1, K=(0.01,0.03)',
    ...
    if with_lpips:
        ...
        perceptual = lpips.LPIPS(net='alex', version='0.1', verbose=False).eval().requires_grad_(False)
        ...
        protocol['lpips'] = dict(network='alex', version='0.1', pretrained=True,
                                 input='native RGB, normalize=True ([0,1] -> [-1,1])',
    ...
    if baseline_dir is not None:
        ...
        if (baseline.get('format') != 'modal_gaussians.result_evaluation' or baseline.get('version') != 1
    ...
    with _publish(destination) as work:
        ...
        with (work / 'per_frame.csv').open('w', encoding='utf-8', newline='') as stream:
            ...
            for view, source, directory in sources:
                ...
                for index, record in enumerate(source['files']):
                    ...
                    values = frame_metrics(render(index, q, 1.0), target.to(device), perceptual)
                    row = dict(view=view['label'], frame_index=index, frame_name=record['name'],
                               ...
                               pixels=camera.height * camera.width, **values)
        ...
        manifest = dict(format='modal_gaussians.result_evaluation', version=1,
