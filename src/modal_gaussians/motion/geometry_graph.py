"""Unfiltered mutual KNN geometry and overlapping control graphs."""
...
class GeometryGraphConfig:
    ...
    def validate(self):
        if self.edge_filter != 'none':
            raise ValueError('Candidate graph must be unfiltered KNN')
        ...
        if self.alpha_minimum > 1:
            raise ValueError('Alpha minimum must be at most one')
...
def build_geometry_graph_arrays(*, foreground_means, view_count, config=None):
    ...
    return GeometryGraph(points=points.astype(np.float32), node_gaussian_index=np.arange(len(points), dtype=np.int64),
