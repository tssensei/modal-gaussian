...
class PerFrequencyModalGNN(nn.Module):
    ...
    def __init__(self, config: NeuralFieldConfig | None = None, *, control_count: int | None = None) -> None:
        ...
        if feature_dim:
            ...
            # Rows follow the immutable saved control order (local to the learning components).
...
def evaluate_model(model_state: Mapping[str, Any], geometry: NeuralFieldGeometry,
    ...
    _validate_finite_payload(model_state, "Neural model")
...
_LOSS_FIELDS = ("field", "rotation", "control_rotation")
...
def _objective(model: PerFrequencyModalGNN, geometry: NeuralFieldGeometry,
    ...
    with torch.set_grad_enabled(backward):
        ...
        values = torch.stack([*view_losses, edge.detach(), rotation.detach()]).cpu().tolist()
        data_value = sum(values[:-2])
        edge_value, rotation_value = values[-2:]
