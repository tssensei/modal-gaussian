...
from modal_gaussians.common.cache import atomic_json, exclusive_work, identity
...
from modal_gaussians.motion.control_preparation import ready as weights_ready
from modal_gaussians.motion.control_propagation import backend_identity
from modal_gaussians.motion.observations.alpha import backend_identity as alpha_backend_identity
    ...
    for index, frequency in zip(bins, frequencies):
        ...
        stages = [
            ...
            ("graph", weights / "manifest.json", ["graph", "build-modal-similarity", "--prepared", str(target),
                ...
                "--minimum-edge-factor", "0.05", "--output", str(weights)]),
...
def _published(job, stage, parent):
    ...
    if name == "prepare":
        ...
        if source["selected_modal_supervision"].get("alpha_backend") != job.get("alpha_contract"):
...
def publish_index(root, jobs):
    """Publish the exact completed bindings consumed by viewing and coefficient fitting."""
    records = []
    for job in jobs:
        experiment = resolve_path(job.get('result_dir', job['stages'][-1][1].parent), strict=True)
        status = _read(experiment / 'status.json')
        if job['state'] != 'complete' or status.get('status') != 'modes_ready':
            raise ValueError('Cannot index an incomplete frequency')
        model = resolve_path(status['outputs']['completed_modes'], strict=True)
        manifest = _read(model / 'manifest.json')
        if (manifest.get('format') != 'modal_gaussians.completed_modes' or manifest.get('version') != 18
                or [m['frequency_hz'] for m in manifest['modes']] != [job['frequency_hz']]):
            raise ValueError('Completed model differs from its batch frequency')
        records.append({'status':'complete', 'frequency_hz':job['frequency_hz'],
            'completed_modes':str(model), 'completed_modes_identity':manifest['completed_modes_identity'],
            'experiment':str(experiment), 'prepared':status['outputs']['prepared']})
    atomic_json(root / 'index.json', records)
...
def _completed_from(source, contract, jobs):
    ...
    if previous.get("alpha_backend") != contract["alpha_backend"]:
        raise ValueError("Resume source has a different alpha backend/revision")
    if previous.get("propagation") != contract.get("propagation"):
...
def _continued_from(source, contract, jobs):
    ...
    from modal_gaussians.motion.continuation import increased_limit, model_contract
    previous = _read(source / "batch_contract.json")
    if previous.get("propagation") != contract.get("propagation"):
    ...
    if (any(previous.get(key) != contract.get(key) for key in keys)
            or any(previous["config"][key] != contract["config"][key] for key in ("fragment",))):
        ...
        for key, path in inputs.items():
            ...
            if key == "prepared_dir":
                job["alpha_contract"] = _read(path / "manifest.json").get("source", {}).get(
                    "selected_modal_supervision", {}).get("alpha_backend")
            old = job[key]
            job[key] = path
...
def run_batch(*, modal_images, prepared_dir, geometry_graph_dir, config_path, output_dir,
              cpu_workers=3, gpu_workers=2, threads_per_worker=2,
    ...
    if not (1 <= cpu_workers <= 8 and 0 <= gpu_workers <= 8) or threads_per_worker < 1:
    ...
    with exclusive_work(root / "batch.lock"):
        ...
        cpu_env = dict(env, CUDA_VISIBLE_DEVICES="-1")
        active, failed = {}, False
        weight_worker = None
        weight_log = None
        weight_request = None
        ...
        def queue(stage):
            return "gpu" if stage in ("train", "weights", "prepare") else "cpu"
        ...
        def save(status):
            ...
            snapshot = {"status": status, "pid": os.getpid(),
                ...
                "phase": ("alpha" if not preparation_barrier(jobs) else
        ...
        with (root / "gpu_usage.csv").open("a", newline="", encoding="utf-8") as resource_log:
            ...
            try:
                while True:
                    ...
                    for job in jobs:
                        ...
                        if failed or counts()[resource] >= limits[resource]:
                            continue
                        if current_stage == "weights" and not preparation_barrier(jobs):
                            continue
                        if current_stage == "train":
                            ...
                            stop_weight_worker()
                        resident_stage = current_stage in ("prepare", "weights")
                        if resident_stage:
                            ...
                            if weight_worker is None:
                                ...
                                weight_worker = subprocess.Popen([sys.executable, "-m",
                                    "modal_gaussians.motion.control_preparation", str(root), str(os.getpid())],
                ...
                save("interrupted")
                raise
        if not failed and stage == "modes":
            publish_index(root, jobs)
        if failed:
