"""Publish modal-image graph experiments from an existing full KNN candidate cache."""
...
from .modal_gradient import ModalGradientConfig, filter_modal_gradient_graph
...
def _modal_view(path, flow, view, frequency):
    ...
    sequence = Path(flow["path"]) / "stabilized_sequence"
    reference = frozen["reference_frame_name"] + ".png"
    if (Path(manifest["stabilized_images"]).resolve() != (sequence / "images").resolve()
            or Path(manifest["reference_image"]).resolve() != (sequence / "images" / reference).resolve()):
    ...
    mask_path = sequence / "masks" / reference
    ...
    return field[0], mask, {"label": view["label"], "path": str(root),
        ...
        "reference_mask": str(mask_path), "prepared_flow_identity": flow["identity"]}


def build_modal_gradient_graph_artifact(*, config=None, **kwargs):
    return _build_modal_graph_artifact(settings=config or ModalGradientConfig(),
        kind="gradient", build_graph=filter_modal_gradient_graph, **kwargs)


def build_modal_similarity_graph_artifact(*, config=None, **kwargs):
    from .modal_similarity import ModalSimilarityConfig, build_modal_similarity_graph
    return _build_modal_graph_artifact(settings=config or ModalSimilarityConfig(),
        kind="similarity", build_graph=build_modal_similarity_graph, **kwargs)


def _build_modal_graph_artifact(*, prepared_dir, geometry_graph_dir, views,
                               frequency_hz, output_dir, settings, kind, build_graph, command=None):
    ...
    timer = Timings()
    with timer.stage(f"modal_{kind}_inputs"):
        ...
        with np.load(prepared_path / "arrays.npz", allow_pickle=False) as archive:
            ...
            for view in selected:
                ...
                field, mask, record = _modal_view(requested[view["label"]],
                    flows[view["flow_identity"]], view, frequency_hz)
                fields.append(field)
                masks.append(mask)
    ...
    with timer.stage(f"modal_{kind}_graph"):
        result, diagnostics, summary = build_graph(graph,
        ...
        summary["unknown_edges_retained"] = int(np.all(result.edge_view_evidence == 0, axis=1).sum())
        for row, view in zip(summary["views"], selected):
            row["label"] = view["label"]
    manifest = {"format": f"modal_gaussians.modal_{kind}_graph", "version": 1,
        ...
        "config": {"graph_neighbors": contract["config"]["graph_neighbors"],
                   ...
                   "graph_edge_filter": f"modal-{kind}", f"modal_{kind}": asdict(settings)},
    ...
    with timer.stage(f"modal_{kind}_publish"):
