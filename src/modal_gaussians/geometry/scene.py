...
from modal_gaussians.common.camera_geometry import PROJECTION_CONVENTION, project_camera, undistort_normalized
...
class Camera:
    ...
    def from_manifest_record(cls, payload: Mapping[str, Any]) -> "Camera":
        ...
        applied = payload["distortion_applied"]
        ...
        if not applied and "projection_convention" in payload:
            raise ValueError("Pinhole camera has contradictory projection metadata")
        ...
        if not np.allclose(
            ...
            raise ValueError(f"Static cameras require unchanged raw/normalized K: {payload.get('name')}")
        camera = cls(
...
def load_static_scene(
    ...
    *, validate: bool = False,
    ...
    if manifest.get("format") != "modal_gaussians.static_scene" or manifest.get("version") not in (2, 3):
    ...
    if validate and _sha256_file(tensors_path) != manifest.get("tensors_sha256"):
